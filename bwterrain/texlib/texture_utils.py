...
class PixelsAdapter(object):
    def __init__(self, img):
        self.img = img 
        #self.pixels = [img.pixels 
        self.size_x, self.size_y = img.size
        self.pixels = [int(val*255) for val in img.pixels[:]]
        
    def __setitem__(self, index, val):
        x, y = index 
        y = self.size_y - y - 1
        pix_index = (x + y*self.size_x)*4
        
        self.pixels[pix_index] = val[0]/255.0
        self.pixels[pix_index+1] = val[1]/255.0
        self.pixels[pix_index+2] = val[2]/255.0
        self.pixels[pix_index+3] = val[3]/255.0
    ...
    def __getitem__(self, index):
        ...
        return self.pixels[pix_index:pix_index+4]
...
class Image(object):
    ...
    @classmethod 
    def new(cls, imgtype, size, bgcolor):
        img = cls()
        img.image = bpy.data.images.new("BWTexture", size[0], size[1])
        return img 
    
    @classmethod
    def from_blender(cls, blenderimg):
        img = cls()
        img.image = blenderimg 
        img.width, img.height = blenderimg.size 
        return img 
    
    def resize(self, dim, texfilter):
        copy = self.image.copy()
        copy._bw_is_copy = True 
        copy.scale(self.width//2, self.height//2)
        img_copy = Image.from_blender(copy)
        return img_copy 
        
    def load(self):
        return PixelsAdapter(self.image)
    
    @property
    def size(self):
        return self.width, self.height 
    
    def convert(self, imgtype):
        return self 
...
def decode_image(image_data, palette_data, image_format, palette_format, num_colors, image_width, image_height):
  ...
  block_x = 0
  block_y = 0
  print("Decoding...", image_width, image_height)
  
  values = [0.0 for i in range(image_width*image_height*4)]
  ...
  while block_y < image_height:
    ...
    for i, color in enumerate(pixel_color_data):
      ...
      if x >= image_width or y >= image_height:
        continue
      index = (x + (image_height - (y+1))*image_width)*4
      #pixels[index:index+4] = (color[0]/255.0, color[1]/255.0, color[2]/255.0, color[3]/255.0)
      values[index] = color[0]/255.0
      values[index+1] = color[1]/255.0
      values[index+2] = color[2]/255.0
      values[index+3] = color[3]/255.0
